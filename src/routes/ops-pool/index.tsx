...
export const Route = createFileRoute("/ops-pool/")({
  component: PlatformResourcePoolOverviewRoute,
});

function PlatformResourcePoolOverviewRoute() {
  const [regions, setRegions] = useState(initialRegions);

  const summary = useMemo(
    () => ({
      openRegions: regions.filter(
        (region) => region.status === "enabled" && region.openForTenant,
      ).length,
      gpuFree: regions.reduce(
        (total, region) => total + region.capacity.gpuFree,
        0,
      ),
      tenantsTotal: regions.reduce(
        (total, region) => total + region.tenantCount,
        0,
      ),
    }),
    [regions],
  ...
  const refreshAll = () => {
    ...
    setRegions((current) =>
      current.map((region) => ({ ...region, updatedAt })),
    );
  ...
  const columns: ListColumn<PlatformRegion>[] = [
    {
      ...
      width: 180,
      render: (_, region) => (
        <DataTableNameCell name={region.name} secondary={region.code} />
      ),
    },
    {
      title: "状态",
    ...
    },
    {
      ...
      render: (_, region) => {
        ...
        const usedPercent = gpuTotal
          ? Math.round(((gpuTotal - gpuFree) / gpuTotal) * 100)
          : 0;
        return (
    ...
    },
    {
      ...
      width: 140,
      render: (_, region) =>
        `${region.capacity.nodes} / ${region.azs.join(", ")}`,
    },
    {
      title: "租户数",
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid grid-cols-4 gap-3.5 max-[1180px]:grid-cols-2">
        ...
        <Metric
          label="租户合计"
          value={String(summary.tenantsTotal)}
          hint="全部区域租户"
        />
        <Metric
          label="GPU 空闲"
          value={String(summary.gpuFree)}
          hint="全部区域汇总"
        />
      </section>
