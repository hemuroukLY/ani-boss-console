...
export const Route = createFileRoute("/overview-capacity/")({
  component: function CapacityOverviewRoute() {
    ...
    return (
      <>
        ...
        <section className="mb-4 grid grid-cols-4 gap-3.5">
          ...
          <Metric label="租户数" value={formatValue(summary?.tenantCount)} hint="已归属租户" />
        </section>
            ...
            {regions.map((region) => (
              <Panel key={region.id} title={regionTitle(region)} action={regionStatus(region)}>
                ...
                <SoftList>
                  ...
                  />
                  <SoftRow title="CPU 核数" meta={formatValue(region.capacity.cpuCores)} />
                  <SoftRow title="内存" meta={`${formatValue(region.capacity.memoryGiB)} GiB`} />
