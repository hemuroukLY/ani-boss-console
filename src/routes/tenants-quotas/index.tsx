...
import type { TenantQuotaLimits } from "@/features/tenant-management/model";
...
export const Route = createFileRoute("/tenants-quotas/")({
  component: QuotaPolicyRoute,
});

function QuotaPolicyRoute() {
  const {
    tenants,
  ...
  const [status, setStatus] = useState<"all" | QuotaPackageStatus>("all");
    ...
    return packages.filter(
      (item) =>
        ...
        (!normalizedKeyword ||
          ...
          item.planCode.toLowerCase().includes(normalizedKeyword)),
    );
