...
import { CreditAdjustmentModal } from "@/components/tenant-management/TenantBillingSummary/CreditAdjustmentModal";
import { formatUsd } from "@/components/tenant-management/TenantBillingSummary/formatters";
import {
  useTenantManagement,
...
interface BillingFilters {
  keyword: string;
  status: "all" | TenantBillingStatus;
  period: string;
}
...
export const Route = createFileRoute("/tenants-billing/")({
  component: TenantBillingListRoute,
});

function TenantBillingListRoute() {
  ...
  const periods = useMemo(
    () =>
      Array.from(new Set(tenantBillings.map((item) => item.period)))
        .sort()
        .reverse(),
    [tenantBillings],
  );
        ...
        <ListPageHeader
          ...
          extra={
            <Button
              icon={<IconDownload />}
              onClick={() =>
                Message.success(`已导出 ${filteredBillings.length} 条计费记录`)
              }
            >
              导出计费记录
            </Button>
            ...
            <ListToolbar
                ...
                <Space wrap>
                  ...
                  <Select
                    ...
                    options={[
                      ...
                      ...Object.entries(tenantBillingStatusMeta).map(([value, meta]) => ({
                        label: meta.label,
                        value,
                      })),
                    ]}
              ...
              tools={
                <Typography.Text type="secondary">
                  共 {filteredBillings.length} 个租户账户
                </Typography.Text>
              }
            />
          }
