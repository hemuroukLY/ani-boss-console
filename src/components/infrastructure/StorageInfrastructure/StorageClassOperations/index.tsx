...
const columns: ListColumn<StorageClassItem>[] = [
  { title: "StorageClass / ID", dataIndex: "name", width: 210, render: (_, item) => <DataTableNameCell name={item.name} secondary={item.id} /> },
  ...
  { title: "默认", dataIndex: "isDefault", width: 90, render: (value: boolean) => (value ? "是" : "否") },
  {
    title: "状态",
    dataIndex: "enabled",
    width: 90,
    render: (value: boolean) => (
      <span className={clsx("inline-flex rounded px-2 py-0.5 text-xs font-medium", value ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-600")}>
        {value ? "已启用" : "已停用"}
      </span>
    ),
  },
      ...
      header={
        <div className="flex items-center justify-between px-5 pt-5">
          <div>
            ...
            <div className="mt-1 text-xs text-gray-500">查看平台存储类与后端、介质和回收策略的映射关系。</div>
          </div>
          <span className="text-xs text-gray-500">共 {storageClasses.length} 个 StorageClass</span>
        </div>
      }
    >
      <ListDataTable rowKey="id" columns={columns} data={storageClasses} pagination={false} scroll={{ x: 1090 }} emptyText="还没有 StorageClass 数据" />
    </ListPageFrame>
  );
}
