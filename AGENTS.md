...
3. 页面信息架构与交互规格以 GitNexus 索引 `产品原型-9.03` 为准；后端接口与行为以 GitNexus 索引 `ANI` 为准。
...
- 新增或修改页面前，必须通过 GitNexus 查询 `产品原型-9.03` 中对应的菜单、路由和页面规格；不得根据现有前端页面反推产品要求。
...
当前前端仓库索引名为 `ani-boss-console`，后端索引名为 `ANI`，产品原型索引名为 `产品原型-9.03`。GitNexus 查询必须使用当前会话接入的工具，不得使用仓库内 CLI 或索引文件替代。
...
- 查看产品原型、页面信息架构或交互布局时，必须使用当前会话已接入的 GitNexus 工具查询产品原型索引 `产品原型-9.03`（`repo: "产品原型-9.03"`）。
- 文档指定的仓库或索引不可用、未建立或无法访问时，不得根据前端代码、训练数据或经验猜测接口契约、后端行为、产品原型或交互布局；必须立即停止相关判断并提示用户建立或恢复对应索引，待索引可用后再继续。

<!-- gitnexus:start -->
# GitNexus — Code Intelligence

This project is indexed by GitNexus as **ani-boss-console** (1330 symbols, 2805 relationships, 105 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
